...
export class RenderPass {
  ...
  public setup() {
    ...
    if (this.textureMapped) {
      if (!this.textureLoaded) {
        ...
        img.src = "/static/assets/minecraft/" + this.textureMap;
      }
    }
  ...
  // Added by Govind
  public updateDrawCount(drawCount: number): void {
    this.drawCount = drawCount;
  }
  ...
  public setAttribData(name: string, data: BufferData): void {
    let attrBuffer = this.attributeBuffers.get(name);
    attrBuffer.data = data;

    const gl: WebGLRenderingContext = this.ctx;
    let attrLoc = gl.getAttribLocation(this.shaderProgram, name);
